...
class NavigationServiceProvider extends ServiceProvider
{
    ...
    private function getNavigationServices()
    {
        return cache()->remember('nav_services', 3600, function () {
            return Post::where('type', 'service')->active()->orderBy('orderposition', 'asc')->orderBy('title', 'asc')->select('id', 'title', 'slug', 'excerpt')->get();
        });
    }
    ...
    private function getNavigationPracticeAreas()
    {
        return cache()->remember('nav_practice_areas', 3600, function () {
            return Post::where('type', 'practice')->active()->orderBy('orderposition', 'asc')->orderBy('title', 'asc')->select('id', 'title', 'slug', 'excerpt')->get();
        });
    }
