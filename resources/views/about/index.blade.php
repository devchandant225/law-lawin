...
@section('content')
    ...
    @if($aboutContent->isNotEmpty())
        @foreach($aboutContent as $content)
            <section class="relative py-16 lg:py-12 bg-gradient-to-b from-gray-50 to-white overflow-hidden">
                <!-- Decorative Background Pattern -->
                <div class="absolute inset-0 opacity-5">
                    <div class="absolute top-0 left-0 w-96 h-96 bg-primary rounded-full mix-blend-multiply filter blur-xl animate-blob"></div>
                    <div class="absolute top-0 right-0 w-96 h-96 bg-secondary rounded-full mix-blend-multiply filter blur-xl animate-blob animation-delay-2000"></div>
                    <div class="absolute -bottom-8 left-20 w-96 h-96 bg-primary/70 rounded-full mix-blend-multiply filter blur-xl animate-blob animation-delay-4000"></div>
                </div>
                ...
                <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                            ...
                            <div class="relative {{ $loop->even ? 'order-2 lg:order-1' : 'order-2 lg:order-2' }}">
                                <div class="relative">
                                    <div class="relative overflow-hidden rounded-xl p-6 sm:p-8 shadow">
                                        <img src="{{ asset('storage/' . $content->image1) }}" 
                                             alt="{{ $content->title }}"
                                             class="w-full h-[30rem] object-cover rounded">
                                        <div class="absolute inset-0 bg-gradient-to-br from-primary/5 via-transparent to-secondary/5 rounded-3xl pointer-events-none"></div>
                                    </div>
                                </div>
                            </div>
                        @endif
                    ...
                    @if($content->image2)
                        <div class="mt-12">
                            <div class="relative overflow-hidden rounded-xl p-6 sm:p-8 shadow">
                                <img src="{{ asset('storage/' . $content->image2) }}" 
                                     alt="{{ $content->title }}"
