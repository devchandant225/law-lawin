...
@section('title', 'Edit Homepage FAQ')

@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div>
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-gray-900">Edit Homepage FAQ</h1>
        ...
        <form action="{{ route('admin.homepage-faqs.update', $homepageFaq) }}" method="POST">
            @csrf
            @method('PUT')
                        ...
                        <div class="p-4 space-y-4">
                            ...
                            <div>
                                ...
                                <textarea id="question" name="question" rows="3" required 
                                          ...
                                          class="block w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-indigo-500 focus:ring-indigo-500 @error('question') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror">{{ old('question', $homepageFaq->question) }}</textarea>
                                @error('question')
                            ...
                            <div>
                                ...
                                <textarea id="answer" name="answer" rows="6" required 
                                          ...
                                          class="block w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-indigo-500 focus:ring-indigo-500 @error('answer') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror">{{ old('answer', $homepageFaq->answer) }}</textarea>
                                @error('answer')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="mb-6 bg-white border border-gray-200 rounded-xl shadow-sm">
                            ...
                            <div class="p-4 space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="number" id="order_index" name="order_index" value="{{ old('order_index', $homepageFaq->order_index) }}" min="0"
                                ...
                                <div>
                                    ...
                                    <select id="status" name="status" required
                                        ...
                                        <option value="active" {{ old('status', $homepageFaq->status) == 'active' ? 'selected' : '' }}>Active</option>
                                        <option value="inactive" {{ old('status', $homepageFaq->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                                    </select>
                                    @error('status')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
