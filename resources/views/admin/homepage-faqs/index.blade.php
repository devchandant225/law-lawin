...
@section('title', 'Homepage FAQ Management')

@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
	<div>
		<div class="flex items-center justify-between mb-6">
			<h1 class="text-2xl font-bold text-gray-900">Homepage FAQ Management</h1>
			<a href="{{ route('admin.homepage-faqs.create') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-white hover:bg-purple-700 focus:ring-2 focus:ring-primary transition-colors">
		...
		<div class="mb-6 bg-white border border-gray-200 rounded-xl shadow-sm">
			<div class="p-4">
				<form method="GET" action="{{ route('admin.homepage-faqs.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
					<div>
					...
					<div class="flex items-end gap-2">
						...
						<a href="{{ route('admin.homepage-faqs.index') }}" class="inline-flex items-center justify-center px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50">Clear</a>
					</div>
				</form>
			</div>
		</div>
			...
			<div class="p-0">
							...
							<tbody class="bg-white divide-y divide-gray-100">
								@foreach($faqs as $faq)
									<tr>
										...
										<td class="px-4 py-3">
											<div class="text-gray-900 font-medium">{{ Str::limit($faq->question, 80) }}</div>
										</td>
										<td class="px-4 py-3">
											@php
												$badgeClasses = [
										...
										<td class="px-4 py-3">
											<div class="flex items-center gap-2">
												<a href="{{ route('admin.homepage-faqs.edit', $faq) }}" title="Edit" class="inline-flex items-center justify-center w-8 h-8 rounded-lg border border-primary text-primary hover:bg-purple-50">
												...
												<form id="delete-form-{{ $faq->id }}" action="{{ route('admin.homepage-faqs.destroy', $faq) }}" method="POST" class="hidden">
													@csrf
													@method('DELETE')
												</form>
											</div>
				...
				@else
					<div class="text-center py-12">
						...
						<a href="{{ route('admin.homepage-faqs.create') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-white hover:bg-purple-700 focus:ring-2 focus:ring-primary">
							<i class="fas fa-plus"></i>
