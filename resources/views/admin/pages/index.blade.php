...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
	<div>
		<div class="flex items-center justify-between mb-6">
			...
			<a href="{{ route('admin.pages.create') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-white hover:bg-purple-700 focus:ring-2 focus:ring-primary transition-colors">
			...
			<div class="p-4">
				<form method="GET" action="{{ route('admin.pages.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
					<div>
						...
						<input type="text" id="search" name="search" value="{{ request('search') }}" placeholder="Search pages..."
					...
					<div>
						...
						<select id="type" name="type" class="block w-full rounded-lg border-gray-300 focus:border-primary focus:ring-primary">
							...
							@foreach(App\Models\Page::getTypes() as $key => $value)
					...
					<div>
						...
						<select id="status" name="status" class="block w-full rounded-lg border-gray-300 focus:border-primary focus:ring-primary">
							...
							@foreach(App\Models\Page::getStatuses() as $key => $value)
					...
					<div class="flex items-end gap-2">
						...
						<a href="{{ route('admin.pages.index') }}" class="inline-flex items-center justify-center px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50">Clear</a>
					</div>
				</form>
			</div>
		</div>
			...
			<div class="p-0">
									...
									<tr>
										<td class="px-4 py-3">
											@if($page->feature_image)
												<img src="{{ $page->feature_image_url }}" alt="{{ $page->feature_image_alt ?: $page->title }}" class="w-12 h-12 rounded object-cover">
											@else
										...
										<td class="px-4 py-3">
											<div class="text-gray-900 font-medium">{{ Str::limit($page->title, 50) }}</div>
											@if($page->excerpt)
												<div class="text-sm text-gray-500">{{ Str::limit($page->excerpt, 80) }}</div>
											@endif
										</td>
										<td class="px-4 py-3">
											<span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs bg-blue-100 text-blue-800">{{ $page->type_name }}</span>
										</td>
										<td class="px-4 py-3">
											<span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs {{ $page->status_badge_class }}">{{ $page->status_name }}</span>
										</td>
										<td class="px-4 py-3 text-gray-700 text-sm">{{ $page->created_at->format('M d, Y') }}</td>
										<td class="px-4 py-3">
											<div class="flex items-center gap-2">
												<a href="{{ route('admin.pages.show', $page) }}" title="View" class="inline-flex items-center justify-center w-8 h-8 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-50">
													<i class="fas fa-eye"></i>
												</a>
												<a href="{{ route('admin.pages.edit', $page) }}" title="Edit" class="inline-flex items-center justify-center w-8 h-8 rounded-lg border border-primary text-primary hover:bg-purple-50">
													<i class="fas fa-edit"></i>
												</a>
												<button type="button" title="Toggle Status" onclick="toggleStatus({{ $page->id }})" class="inline-flex items-center justify-center w-8 h-8 rounded-lg border border-blue-300 text-blue-600 hover:bg-blue-50">
													<i class="fas fa-toggle-{{ $page->status == 'active' ? 'on' : 'off' }}"></i>
												</button>
												<button type="button" title="Delete" onclick="confirmDelete('{{ $page->id }}', '{{ $page->title }}')" class="inline-flex items-center justify-center w-8 h-8 rounded-lg border border-red-300 text-red-600 hover:bg-red-50">
				...
				@else
					<div class="text-center py-12">
						...
						<a href="{{ route('admin.pages.create') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary text-white hover:bg-purple-700 focus:ring-2 focus:ring-primary">
							<i class="fas fa-plus"></i>
