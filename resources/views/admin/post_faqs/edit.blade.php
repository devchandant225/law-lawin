...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold text-gray-900">Edit FAQ for "{{ $post->title }}"</h1>
        ...
        <div class="p-6">
            ...
            <form action="{{ route('admin.posts.faqs.update', [$post->id, $faq->id]) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="question" id="question" class="block w-full rounded-lg border-gray-300 focus:border-primary focus:ring-primary @error('question') border-red-500 @enderror" rows="3" placeholder="Enter the FAQ question">{{ old('question', $faq->question) }}</textarea>
                    @error('question')
                ...
                <div class="mb-6">
                    ...
                    <textarea name="answer" id="answer" class="block w-full rounded-lg border-gray-300 focus:border-primary focus:ring-primary @error('answer') border-red-500 @enderror" rows="5" placeholder="Enter the FAQ answer">{{ old('answer', $faq->answer) }}</textarea>
                    @error('answer')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
