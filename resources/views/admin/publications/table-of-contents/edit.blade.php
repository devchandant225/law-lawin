            ...
            <form action="{{ route('admin.publications.table-of-contents.update', [$publication, $tableOfContent]) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <select class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('status') border-red-500 @enderror" 
                                id="status" 
                                name="status">
                ...
                <div class="mb-6">
                    ...
                    <textarea class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('description') border-red-500 @enderror" 
                              id="description" 
                              name="description" 
                              rows="5" 
    ...
    <div class="bg-white shadow rounded-lg mt-6">
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                    <div class="flex items-center">
                        <i class="fas fa-toggle-{{ $tableOfContent->status ? 'on' : 'off' }} {{ $tableOfContent->status ? 'text-green-500' : 'text-gray-400' }} mr-3"></i>
                        <div>
                            <h4 class="text-sm font-medium text-gray-900">Status: {{ $tableOfContent->status ? 'Active' : 'Inactive' }}</h4>
                            <p class="text-xs text-gray-500">Toggle item status</p>
                        </div>
                    </div>
                    <div>
                        <button class="inline-flex items-center px-3 py-2 border {{ $tableOfContent->status ? 'border-yellow-300 text-yellow-600 hover:bg-yellow-50' : 'border-green-300 text-green-600 hover:bg-green-50' }} text-sm rounded-md focus:ring-2 focus:ring-{{ $tableOfContent->status ? 'yellow' : 'green' }}-500 transition-colors" 
            ...
            <div class="border-t border-gray-200 pt-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                        ...
                        <div>
                            <button class="inline-flex items-center px-3 py-2 border border-red-300 text-red-600 text-sm rounded-md hover:bg-red-50 focus:ring-2 focus:ring-red-500 transition-colors" onclick="confirmDelete()">
                                Delete
                            </button>
                        </div>
                    </div>
...
<div class="fixed inset-0 z-50 hidden" id="deleteModal">
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="relative bg-white rounded-lg max-w-md w-full shadow-xl">
            ...
            <div class="px-6 py-4">
                <p class="text-sm text-gray-700 mb-2">Are you sure you want to delete "<strong>{{ $tableOfContent->title }}</strong>"?</p>
                <p class="text-sm text-red-600">This action cannot be undone.</p>
            </div>
            <div class="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
                <button type="button" class="inline-flex items-center px-4 py-2 border border-gray-300 text-gray-700 rounded-md hover:bg-gray-50 focus:ring-2 focus:ring-gray-500 transition-colors" onclick="hideDeleteModal()">Cancel</button>
                <button type="button" class="inline-flex items-center px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 focus:ring-2 focus:ring-red-500 transition-colors" onclick="deleteItem()">Delete</button>
            </div>
        </div>
    </div>
</div>
