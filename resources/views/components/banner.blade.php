    ...
    <div class="banner-carousel relative h-screen">
        @if(count($sliders) > 0)
            @foreach ($sliders as $index => $slider)
                <div class="carousel-slide {{ $index === 0 ? 'active' : '' }} absolute inset-0 opacity-0 transition-opacity duration-1000 ease-in-out" data-slide="{{ $index }}">
                    <!-- Background Image -->
                    ...
                    <div class="relative z-10 h-full flex items-center">
                        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
                            <div class="max-w-4xl">
                                <!-- Animated Content -->
                                <div class="transform translate-y-8 opacity-0 slide-content transition-all duration-1000 ease-out delay-300">
                                    <!-- Title -->
                                    ...
                                    <!-- Description -->
                                    <p class="text-lg sm:text-xl lg:text-2xl text-gray-200 mb-8 max-w-3xl leading-relaxed">
                                        {{ $slider->description }}
                                    </p>
        ...
        @if(count($sliders) > 1)
            ...
            <div class="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-20">
                <div class="flex space-x-3">
                    @foreach ($sliders as $index => $slider)
                        <button class="dot w-3 h-3 rounded-full transition-all duration-300 {{ $index === 0 ? 'bg-white scale-125' : 'bg-white/50 hover:bg-white/75' }}" 
            ...
            <!-- Navigation Arrows -->
            <button class="prev-btn absolute left-4 sm:left-8 top-1/2 transform -translate-y-1/2 z-20 w-12 h-12 sm:w-14 sm:h-14 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white hover:bg-white/30 transition-all duration-300 group" 
            ...
            <button class="next-btn absolute right-4 sm:right-8 top-1/2 transform -translate-y-1/2 z-20 w-12 h-12 sm:w-14 sm:h-14 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white hover:bg-white/30 transition-all duration-300 group" 
        ...
        @if(count($sliders) > 1)
            <div class="absolute bottom-0 left-0 w-full h-1 bg-white/20 z-20">
                <div class="progress-bar h-full bg-white transition-all duration-300 ease-linear" style="width: 0%"></div>
            </div>
        @endif
    </div>
