        ...
        <div class="container" style="position: relative; z-index: 1; padding: 80px 15px;">
                            ...
                            ">
                                ...
                                @if(!empty($search))
                                    <div class="search-info mt-2" style="color: #fff; font-size: 14px; text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.8);">
                                        <span>{{ $languages->total() }} language(s) found for "{{ $search }}"</span>
                                        <button 
                            ...
                            <div class="award-one__item">
                                <div class="award-one__item__left">
                                    <h2 class="award-one__item__title">
                                        <a style="color:#fff"
                                            href="{{ route('languages.show', $language->slug) }}">{{ $language->title }}</a>
                                    </h2>
                                    @if ($language->excerpt || $language->description)
                                        <p class="award-one__item__text">
                                            {{ $language->excerpt ?? Str::limit(strip_tags($language->description), 150) }}
                                        </p>
                                    @endif
                                    <div class="language-meta mt-3">
            ...
            @else
                <div class="text-center py-5" wire:loading.remove.delay>
                    @if(!empty($search))
                        <h4 style="color: #fff; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);">No languages found for "{{ $search }}"</h4>
                        <p style="color: #fff; text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.8);">Try searching with different keywords or browse all languages.</p>
                        <button 
                            wire:click="$set('search', '')"
                            class="procounsel-btn procounsel-btn--two mt-3"
                        >
                            <i>Show All Languages</i>
                            <span>Show All Languages</span>
                        </button>
                    @else
                        <h4 style="color: #fff; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);">No language services available at the moment.</h4>
                        <p style="color: #fff; text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.8);">Please check back later for our language service updates.</p>
                    @endif
                </div>
            @endif

            @if ($showViewAll && $languages->count() > 0)
                <div class="text-center mt-5" wire:loading.remove.delay>
                    <a href="{{ route('languages.index') }}" class="procounsel-btn procounsel-btn--two">
                        <i>View All Languages</i>
