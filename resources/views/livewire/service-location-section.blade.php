<div>
    <section class="award-one bg-section" style="
        position: relative;
        min-height: 100vh;
        background-image: url('/service-location.jpg');
    ...
    ">
        ...
        <div class="container" style="position: relative; z-index: 1; padding: 80px 15px;">
                    ...
                    <div class="row justify-content-center">
                        <div class="col-lg-6 col-md-8">
                            ...
                            ">
                                ...
                                @if(!empty($search))
                                    <div class="search-info mt-2" style="color: #fff; font-size: 14px; text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.8);">
                                        <span>{{ $serviceLocations->total() }} location(s) found for "{{ $search }}"</span>
                                        <button 
                                            wire:click="$set('search', '')"
                                            class="btn btn-sm ms-2" 
                                            style="color: #fff; text-decoration: underline; border: none; background: none; padding: 0;"
                                            type="button"
                                        >
                                            Clear
                                        </button>
            ...
            @if ($serviceLocations->count() > 0)
                            ...
                            <div class="award-one__item">
                                <div class="award-one__item__left">
                                    <h2 class="award-one__item__title">
                                        <a style="color:#fff"
                                            href="{{ route('service-locations.show', $serviceLocation->slug) }}">{{ $serviceLocation->title }}</a>
                                    </h2>
                                    @if ($serviceLocation->excerpt || $serviceLocation->description)
                                        <p class="award-one__item__text">
                                            {{ $serviceLocation->excerpt ?? Str::limit(strip_tags($serviceLocation->description), 150) }}
                                        </p>
                                    @endif
                                    <div class="service-location-meta mt-3">
                                        <div class="d-flex align-items-center text-white-50 mb-2">
                                            <i class="fas fa-map-marker-alt me-2"></i>
                                            <span>Service Location</span>
                                        </div>
                                        <a href="{{ route('service-locations.show', $serviceLocation->slug) }}" 
                ...
                <!-- Pagination -->
                @if($serviceLocations->hasPages())
                    <div class="row mt-5" wire:loading.remove.delay>
                        <div class="col-12">
            ...
            @else
                <div class="text-center py-5" wire:loading.remove.delay>
                    @if(!empty($search))
                        <h4 style="color: #fff; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);">No service locations found for "{{ $search }}"</h4>
                        <p style="color: #fff; text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.8);">Try searching with different keywords or browse all locations.</p>
                        <button 
                            wire:click="$set('search', '')"
                            class="procounsel-btn procounsel-btn--two mt-3"
                        >
                            <i>Show All Locations</i>
                            <span>Show All Locations</span>
                        </button>
                    @else
                        <h4 style="color: #fff; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8);">No service locations available at the moment.</h4>
                        <p style="color: #fff; text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.8);">Please check back later for our service location updates.</p>
                    @endif
                </div>
            @endif

            @if ($showViewAll && $serviceLocations->count() > 0)
                <div class="text-center mt-5" wire:loading.remove.delay>
                    <a href="{{ route('service-locations.index') }}" class="procounsel-btn procounsel-btn--two">
                        <i>View All Service Locations</i>
    ...
    <style>
        ...
        /* Service location specific styling */
        .service-location-meta {
            margin-top: 15px;
        }

        .service-location-meta .fas {
